...
export type TWeatherMode = keyof typeof weatherIcons
export type TWeatherIcon =
  | keyof typeof weatherIcons['day']
  | keyof typeof weatherIcons['night']
...
const weatherCodeToType = (weatherCode: number): TWeatherIcon => {
  const weatherType = Object.entries(weatherTypes).find(([_, weatherCodes]) => {
    return weatherCodes.includes(weatherCode)
  })
  return weatherType![0] as TWeatherIcon
}

function WeatherIcon({ weatherCode, moment }: IProps) {
  const weatherType: TWeatherIcon = useMemo(
